...
class BubbleSortGUI : public AlgorithmGUI {
 ...
 public:
  ...
  void showValues(ImVec2 pos, ImVec2 size) const {
    ...
    const float width = content_size.x / static_cast<float>(values.size());
    ...
    auto draw_value_rect = [&](std::size_t i, ImU32 col) {
      ...
      draw_list->AddRectFilled(
          ImVec2(content_min_p.x + static_cast<float>(i + 1) * width,
                 content_max_p.y - height),
          ImVec2(content_min_p.x + static_cast<float>(i) * width, content_max_p.y), col);
    };
...
/*        for (std::size_t i = 0; i < values.size(); ++i) {
  ...
  void show(ImVec2 pos, ImVec2 size) override {
    showControlPanel(ImVec2(pos.x, pos.y + size.y - 100), ImVec2(size.x, 100));
    if (bubblesort_coroutine.has_value()) {
      if (play) {
        static float spare_time = 0.0f;
        ImGuiIO &io = ImGui::GetIO();
        int nb_steps = static_cast<int>((spare_time + io.DeltaTime) * steps_per_s);
        spare_time = (spare_time + io.DeltaTime) * steps_per_s - static_cast<float>(nb_steps);
